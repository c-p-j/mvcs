...
class modelLanguage
{
    ...
    public static function insert($row)
    {
        ...
        $sqlText = "INSERT INTO language (
                    `language_id`,
                    `name`,
                    `last_update`)
        VALUES (" . $row->getlanguage_id()
            . ",'" . $row->getname() . "',"
            . "'" . $row->getlast_update() . "')";
    ...
    public static function select($where, $orderBy)
    {
        ...
        try {
            ...
            foreach ($result as $row) {
                array_push($dataset, new dataobjLanguage($row["language_id"], $row["name"], $row["last_update"]));
            }
            return $dataset;
        } catch (PDOException $e) {
